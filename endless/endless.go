...
/*
NewTcpServer returns an intialized endlessServer Object. Calling Serve on it will
actually "start" the server.
*/
func NewTcpServer(addr string, handler TcpHandler) (srv *endlessServer) {
	runningServerReg.Lock()
	defer runningServerReg.Unlock()
...
func (srv *endlessServer) tcpServer() error {
	for {
		c, err := srv.EndlessListener.Accept()
		if err != nil {
			return err
		}
		srv.tcpHandler(c)
...
func (srv *endlessServer) getListener(laddr string) (l net.Listener, err error) {
	if srv.isChild {
		...
		if err != nil {
			err = fmt.Errorf("net.Listen error: %v", err, " pid:", os.Getpid())
			return
		}
	}
	return
