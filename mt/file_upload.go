...
func CheckFileExist(host, filename, hash, app_id, app_key, id string) *base.OkJson {
	...
	headers["X-Mt-hash"] = hash
	headers["X-Mt-AppId"] = app_id
	if id != "" {
		headers["X-Mt-Id"] = id
	}

	if app_key == "439081c403882a0c86fbed7ce2b4932cfcad47e1" {
		headers["X-Mt-SIGN"] = app_key
		app_key = ""
	}
...
func PostFile2YgUpload(host, filename, app_id, app_key, id, resize, target string, timeout time.Duration, is_test bool) *base.OkJson {
	...
	headers["X-Mt-hash"] = hash
	headers["X-Mt-AppId"] = app_id
	headers["X-Mt-width"] = strconv.Itoa(size.Width)
	headers["X-Mt-height"] = strconv.Itoa(size.Height)

	if id != "" {
		headers["X-Mt-Id"] = id
	}
	if resize != "" {
		headers["X-Mt-resize"] = resize
	}
	if target != "" {
		headers["X-Mt-target"] = target
	}
	if is_test {
		headers["X-Mt-Test"] = "1"
	}
	...
	if app_key == "super" {
		headers["X-Mt-SIGN"] = "eba0cb9dc8cffd641be5d01969674a30"
		app_key = ""
	}

	res = YgHttpPost(uri, content, headers, timeout, app_key)
...
func web_chunk_upload(host string, bodyfile *os.File, headers map[string]string, filesize int64, chunksize, chunkindex int, timeout time.Duration) (*ChunkResponse, error) {
	...
	filename := headers["X-Mt-filename"]
	body := &bytes.Buffer{}
	...
	headers["X-Mt-chunksize"] = strconv.Itoa(chunksize_real)
	headers["X-Mt-chunkindex"] = strconv.Itoa(chunkindex)
	headers["X-Mt-chunkhash"] = chunkhash
...
func YfChunkUploadWeb(host, filename, app_id, id, token string, timeout time.Duration, chunksize int, is_test bool) (*ChunkResponse, error) {
	...
	headers["X-Mt-Platform"] = "web"
	headers["X-Mt-hash"] = hash
	headers["X-Mt-filename"] = url.QueryEscape(filename)
	headers["X-Mt-filesize"] = strconv.FormatInt(filesize, 10)
	if chunksize > 0 {
		headers["X-Mt-chunksize"] = strconv.Itoa(chunksize)
	}
	headers["X-Mt-AppId"] = app_id
